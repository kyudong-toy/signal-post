...
@Slf4j
public class MediaFileValidationUtils {
	...
	public static void validateFileTypeConsistency(String originalFileName, String mimeType, Long uploaderId) {
		if (originalFileName == null || mimeType == null) {
			...
			throw new InvalidMediaFileException("파일 정보가 올바르지 않습니다.");
		}
		...
		if (!ALLOWED_EXTENSIONS.contains(fileExtension)) {
			...
			throw new InvalidMediaFileException("허용되지 않은 파일 형식입니다.");
		}
		...
		if (!allowedExtensions.contains(fileExtension)) {
			...
			throw new InvalidMediaFileException("파일 확장자와 내용이 일치하지 않습니다.");
		}
	...
	private static String extractFileExtension(String fileName) {
		if (fileName == null || !fileName.contains(".")) {
			throw new InvalidMediaFileException("파일 확장자가 없습니다.");
		}

		int lastDotIndex = fileName.lastIndexOf(".");
		if (lastDotIndex == fileName.length() - 1) {
			throw new InvalidMediaFileException("파일 확장자가 올바르지 않습니다.");
		}
	...
	public static void validateFileName(String fileName) {
		if (!StringUtils.hasText(fileName)) {
			throw new InvalidMediaFileException("파일명이 비어있습니다.");
		}
		...
		for (String dangerousChar : dangerousChars) {
			if (fileName.contains(dangerousChar)) {
				throw new InvalidMediaFileException("파일명에 허용되지 않은 문자가 포함되어 있습니다.");
			}
		}

		if (fileName.length() > 255) {
			throw new InvalidMediaFileException("파일명이 너무 깁니다.");
		}
	}
