...
public class DefaultChatEventHandler implements ChatEventHandler {
	...
	public void handleMessageCreate(ChatMessageCreateEvent event) {
		...
		simpMessagingTemplate.convertAndSend("/topic/chatroom/" + event.roomId(), webSocketMessage);
	...
	public void handleRoomCreate(ChatRoomCreateEvent event) {
		...
		event.chatMembers().forEach(member -> {
			ChatWebSocketMessage<ChatNewRoomWsDto> webSocketMessage =
					ChatWebSocketMessage.of(ChatEventType.NEW_CHAT_ROOM, ChatNewRoomWsDto.of(event, member));
	...
	public void handleMemberInvite(ChatMemberInviteEvent event) {
		...
		event.newMembers().forEach(member -> {
			ChatWebSocketMessage<ChatMemberInviteWsDto> webSocketMessage =
					ChatWebSocketMessage.of(ChatEventType.NEW_CHAT_ROOM, ChatMemberInviteWsDto.of(event, member));
			simpMessagingTemplate.convertAndSendToUser(
					member.getUser().getUsername(),
					"/queue/chatroom",
					webSocketMessage
		...
		simpMessagingTemplate.convertAndSend("/topic/chatroom/" + event.roomId(), webSocketSameMessage);
